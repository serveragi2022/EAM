...
const HoldBTWFTWScreen = ({ route }) => {
  ...
  useEffect(() => {
    const title = category === "FW" ? "Certify Fit to Work" : "Confirm Back to Work";
  ...
  const fetchData = async () => {
    ...
    const apiUrl =
      category === "FW"
        ? `${url}/employeeschedule/holdftw?branch=${getGBranch()}`
        : `${url}/employeeschedule/holdbtw?branch=${getGBranch()}&access_department=${encodeURIComponent(
            getGAccessDeptEs()
 ...
 const handleSearch = (text) => {
  ...
  const RenderItem = React.memo(({ item }) => (
    <TouchableOpacity
      ...
      onPress={() =>
        navigation.navigate(category === "FW" ? "FittoWork" : "BacktoWork", {
          category,
          emp_id: item.emp_id,
        })
      }
...
export default HoldBTWFTWScreen;
