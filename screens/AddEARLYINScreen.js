...
import { getGBranch, getGAccessSubBranch, getGAccessSubDeptEs } from "../GlobalVariable";
import { url, headers } from "../api/httpclient";
import { useFocusEffect, useNavigation } from "@react-navigation/native";

const AddEARLYINScreen = () => {
 ...
 const applyFilters = (employees, employer, search, dateTab) => {
  ...
  const fetchData = async () => {
    ...
    const apiUrl = `${url}/employeeschedule/earlyin?branch=${getGBranch()}&access_subbranch=${getGAccessSubBranch()}&access_sub_dept=${getGAccessSubDeptEs()}`;
    try {
    ...
    <View style={{ flex: 1 }}>
      ...
      <View style={styles.filterRow}>
        {dateTabs.map((tab) => (
          <TouchableOpacity
            key={tab}
            style={[styles.filterButton, selectedDateTab === tab && styles.activeButton]}
      ...
      <View style={styles.filterRow}>
        {employers.map((emp) => (
          <TouchableOpacity
            key={emp}
            style={[styles.filterButton, selectedEmployer === emp && styles.activeButton]}
...
export default AddEARLYINScreen;
